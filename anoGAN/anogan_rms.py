...
def generator_model():
    ...
    generator.add(Dense(64 * 123, input_dim=20, kernel_initializer=initializers.RandomNormal(stddev=0.02)))
    generator.add(LeakyReLU(0.2))
    # generator.add(Reshape((7, 7, 128)))
    generator.add(Reshape((123, 64)))
...
def discriminator_model():
    ...
    discriminator.add(Conv1D(32, kernel_size=5, strides=2, padding='same', input_shape=(492, 1), kernel_initializer=initializers.RandomNormal(stddev=0.02)))
...
def anomaly_detector():
    ...
    aInput = Input(shape=(20,))
    # gInput = Dense((100))(aInput)
    gInput = Dense((20))(aInput)
    G_out = g(gInput)
...
def compute_anomaly_score(model, x):    
    ...
    z = np.random.uniform(0, 1, size=(1, 20))
    intermidiate_model = feature_extractor()
