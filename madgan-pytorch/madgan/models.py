...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        extra_features = self.hidden_units if self.add_batch_mean else 0
            # print("discriminator lstm setting: ", self.input_dim, self.hidden_units + extra_features, self.n_lstm_layers)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.add_batch_mean:
            ...
            x = torch.cat([x, batch_mean], dim=-1)
                # print("discriminator forward: ", batch_mean.shape, x.shape)
