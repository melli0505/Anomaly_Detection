...
def train_one_epoch(generator: nn.Module,
    ...
    for i, (real, z) in enumerate(zip(real_dataloader, latent_dataloader)):
        bs = real.size(0) * real.size(1)
        real_labels = torch.full((bs, ), normal_label).float().to(torch.device("cuda")) #.to(real.device)
        fake_labels = torch.full((bs, ), anomaly_label).float().to(torch.device("cuda")) #.to(real.device)
        all_labels = torch.cat([real_labels, fake_labels])

        # Generate fake samples with the generator
        fake = generator(z[:real.size(0)].to(torch.device("cuda")))

            # print("fake, real.shape : ", fake.shape, real.shape)
        # Update discriminator
        discriminator_optimizer.zero_grad()
        discriminator.train()
        real_logits = discriminator(real.to(torch.device("cuda")))
        fake_logits = discriminator(fake.detach())
        d_logits = torch.cat([real_logits, fake_logits[:len(fake)]])

        # Discriminator tries to identify the true nature of each sample
        # (anomaly or normal)

            # print("d loss :", real_logits.view(-1).shape, real_labels.shape)
        d_real_loss = loss_fn(real_logits.view(-1), real_labels)
            # print("d fake loss :", fake_logits.view(-1).shape, fake_labels.shape)
        ...
        if (i + 1) % log_every == 0:
            ...
            print("log every: ", g_logits.shape, real_labels.shape)
