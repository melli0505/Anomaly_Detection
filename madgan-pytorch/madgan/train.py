...
from madgan import preprocessing
...
def train(
    ...
    df = preprocessing.read_data(input_data)
    df = preprocessing.normalize(df)
    df = df[:-1 * (df.shape[0] % constants.LATENT_SPACE_DIM)].reshape(df.shape[0] // constants.LATENT_SPACE_DIM, 512, constants.LATENT_SPACE_DIM)
    ...
    print("generator setting : ", constants.LATENT_SPACE_DIM, hidden_dim, df.shape[-1])
    ...
    print("discriminator setting : ", df.shape[-1], hidden_dim)
